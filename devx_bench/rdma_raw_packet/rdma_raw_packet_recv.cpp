...
WqHandler *create_wq(NetParam &net_param, ibv_pd *pd, void *buf, size_t size) {
    ...
    wq_handler->recv_bar_wr = recv_bar_wr;

    wq_handler->max_inline_size = max_inline_size;
    wq_handler->num_wrs = num_wrs;
    wq_handler->num_sges_per_wr = num_sges_per_wr;
...
void sub_recv_server(int thread_index, WqHandler *handler, size_t ops) {
    ...
    OffsetHandler recv(NB_RXD, PKT_BUF_SIZE, BUF_SIZE / 2);
    OffsetHandler recv_comp(NB_RXD, PKT_BUF_SIZE, BUF_SIZE / 2);

    size_t rx_depth = handler->rx_depth;

    for (size_t i = 0;i < min(rx_depth, ops); i++) {
        handler->recv_sge_list[0].addr = recv.index_mod() * PKT_BUF_SIZE + local_mr_addr;
        ...
        handler->recv_wr->wr_id = recv.index_mod() * PKT_BUF_SIZE + local_mr_addr;
    ...
    double speed_pps = ops / duration / 1e6;
...
void benchmark(NetParam &net_param) {
    ...
    size_t ops = size_t(1) * ITERATIONS * NB_RXD;
    LOG_I("OPS : [%ld]", ops);

    BUF_SIZE = 2 * NB_RXD * PKT_BUF_SIZE;
    ...
    flow_spec_eth->val.ether_type = htons(0x80);
    ...
    // TODO check maybe don't need change to RTR/RTS

    vector<thread> threads(NUM_THREADS);
    for (int i = 0;i < NUM_THREADS;i++) {
        int now_index = get_cpu_index_with_numa(i + CORE_OFFSET, net_param.numa_node);
        threads[i] = thread(sub_recv_server, now_index, wq_handlers[i], ops);
