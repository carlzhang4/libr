...
void sub_task_server(int thread_index, QpHandler *handler, vhca_resource *resource, size_t ops) {
    ...
    size_t host_mr_addr = reinterpret_cast<size_t>(resource->addr);
    size_t local_mr_addr = reinterpret_cast<size_t>(handler->buf);
    size_t host_rkey = resource->mr->lkey;
    ...
    for (size_t i = 0; i < min(size_t(rx_depth), ops);i++) {
        ...
        handler->recv_sge_list[1].length = PACK_SIZE;
        handler->recv_sge_list[1].lkey = host_rkey;
        handler->recv_wr->num_sge = 2;

        handler->recv_wr->wr_id = recv.index_mod() * HEADER_SIZE + local_mr_addr;
        handler->recv_wr->next = NULL;
    ...
    while (!done && !stop_flag) {
        ...
        for (int i = 0;i < ne_recv;i++) {
            ...
            assert(wc_recv[i].byte_len == static_cast<uint32_t>(PACK_SIZE) + HEADER_SIZE);
            assert(*(uint64_t *)(wc_recv[i].wr_id) == recv_comp.index());
            recv_comp.step();
        }
...
void sub_task_client(int thread_index, QpHandler *handler, vhca_resource *resource, size_t ops) {
    sleep(2);
    assert(latency_hist);
    wait_scheduling(thread_index, IO_LOCK);
    ...
    for (size_t i = 0; i < min(tx_depth, ops);i++) {
        handler->send_sge_list[0].addr = send.index_mod() * HEADER_SIZE + local_mr_addr;
        handler->send_sge_list[0].length = HEADER_SIZE;
        handler->send_sge_list[0].lkey = local_rkey;

        handler->send_sge_list[1].addr = send.index_mod() * PACK_SIZE + host_mr_addr;
        handler->send_sge_list[1].length = PACK_SIZE;
        handler->send_sge_list[1].lkey = host_rkey;
        handler->send_wr->num_sge = 2;
        handler->send_wr->wr_id = send.index_mod() * HEADER_SIZE + local_mr_addr;
        handler->send_wr->next = NULL;

        *(uint64_t *)(handler->send_sge_list[0].addr) = send.index();
...
void benchmark(NetParam &net_param) {
    ...
    for (int i = 0;i < NUM_THREADS;i++) {
        ...
        if (net_param.nodeId == 0) {
            threads[i] = thread(sub_task_server, now_index, qp_handlers[i], resources + i, ops);
        } else if (net_param.nodeId == 1) {
            threads[i] = thread(sub_task_client, now_index, qp_handlers[i], resources + i, ops);
        }
...
int main(int argc, char *argv[]) {
    ...
    // all send/recv wr use two segment
    // as for 64B for arm local memory region, other for host payload
    // same as recv side 
    net_param.sge_per_wr = 2;
    // a trick
    rt_assert(PACK_SIZE >= 64);
    rt_assert(FLAGS_serverIp.size() > 0, "serverIp is empty");
    ...
    // first connect to vhca master IP
    net_param.nodeId = 1;
    net_param.serverIp = FLAGS_vhcaIp;
