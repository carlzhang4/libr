...
static uint8_t CLIENT_MAC_ADDR[6] = { 0x98, 0x03, 0x9b, 0xca, 0x48, 0x38 };
static uint8_t SERVER_MAC_ADDR[6] = { 0x98, 0x03, 0x9b, 0xc7, 0xc8, 0x18 };
...
static uint32_t HANDLE_BATCH = 16;
static uint32_t SEND_OUTSTANDING = 256;
...
WqHandler *create_wq(NetParam &net_param, ibv_pd *pd, void *buf, size_t size) {
    ...
    struct ibv_qp_init_attr_ex tx_qp_init_attr_ex;

    memset(&tx_qp_init_attr_ex, 0, sizeof(tx_qp_init_attr_ex));
    tx_qp_init_attr_ex.send_cq = send_cq;
    tx_qp_init_attr_ex.recv_cq = recv_cq;
    tx_qp_init_attr_ex.cap.max_send_wr = tx_depth;
    tx_qp_init_attr_ex.cap.max_send_sge = 1;
    tx_qp_init_attr_ex.qp_type = IBV_QPT_RAW_PACKET;

    tx_qp_init_attr_ex.pd = pd;
    tx_qp_init_attr_ex.create_flags = 0;
    tx_qp_init_attr_ex.max_tso_header = 56;
    tx_qp_init_attr_ex.comp_mask = IBV_QP_INIT_ATTR_PD | IBV_QP_INIT_ATTR_MAX_TSO_HEADER;
    assert(send_qp = ibv_create_qp_ex(net_param.contexts[0], &tx_qp_init_attr_ex));
...
void sub_send_server(int thread_index, WqHandler *handler) {
    ...
    for (int i = 0;i < handler->num_wrs;i++) {
        ...
        handler->send_wr[i].next = nullptr;
        handler->send_wr[i].send_flags = IBV_SEND_IP_CSUM;
        handler->send_wr[i].opcode = IBV_WR_TSO;
        handler->send_wr[i].tso.hdr_sz = 42;
        handler->send_wr[i].tso.mss = 64;
    }
    ...
    while (!stop_flag) {
        if ((send.index() - send_comp.index()) < (SEND_OUTSTANDING - HANDLE_BATCH)) {
            for (size_t i = 0;i < HANDLE_BATCH;i++) {
                ...
                handler->send_wr[i].tso.hdr = reinterpret_cast<void *>(send.offset() + local_mr_addr);
                if (i == HANDLE_BATCH - 1) {
                    handler->send_wr[i].next = nullptr;
