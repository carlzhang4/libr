...
void sub_task_server(int thread_index, QpHandler *handler, size_t ops) {
...
void sub_task_client(int thread_index, QpHandler *handler, size_t ops) {
    sleep(2);
    assert(latency_hist);
    wait_scheduling(thread_index, IO_LOCK);
...
void benchmark(NetParam &net_param) {
    ...
    for (int i = 0;i < NUM_THREADS;i++) {
        ...
        if (resources != nullptr) {
            // devx_mr
            devx_dereg_mr(reinterpret_cast<devx_mr *>(qp_handlers[i]->mr));
        } else {
            // common mr
