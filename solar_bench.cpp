...
inline void do_solar_handler(void *buf, size_t pkt_size, void *dst_buf, OffsetHandler &recv_comp) {
...
inline void do_solar_extra_handler(OffsetHandler &recv_comp) {
    if (FLAGS_type == TestType::CPU) {
        return;
    } else if (FLAGS_type == TestType::CPU_CRCOffload) {
        return;
...
void sub_task_server(int thread_index, QpHandler *handler, void *buf, size_t ops) {
    ...
    while (!done && !stop_flag) {
        ...
        for (int i = 0;i < ne_recv;i++) {
            ...
            do_solar_handler(reinterpret_cast<void *>(handler->buf + wc_recv[i].wr_id), PACK_SIZE, reinterpret_cast<void *>(reinterpret_cast<uint64_t>(copy_bufs) + wc_recv[i].wr_id), recv_comp);
        }

        do_solar_extra_handler(recv_comp);

        while (send.index() < recv_comp.index() && (send.index() - send_comp.index()) < tx_depth) {
            post_send(*handler, send.offset(), 0);
            send.step();
        }
...
void sub_task_client(int thread_index, QpHandler *handler, void *buf, size_t ops) {
    ...
    while ((send_comp.index() < ops || recv_comp.index() < ops) && !stop_flag) {
        ...
        for (int i = 0;i < ne_recv;i++) {
            ...
            if (wc_recv[i].byte_len != static_cast<uint32_t>(0)) {
